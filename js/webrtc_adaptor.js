...
function WebRTCAdaptor(initialValues)
{
	...
	this.askforFloor = function (streamId,user)
	{
		var jsCmd = {
				command : "askforFloor",
				streamId : streamId,
	...
	this.giveFloor = function (streamId)
	{
		var jsCmd = {
				command : "giveFloor",
				streamId : streamId,
	...
	this.getBackFloor = function (streamId)
	{
		var jsCmd = {
				command : "getBackFloor",
				streamId : streamId,
		}
		thiz.webSocketAdaptor.send(JSON.stringify(jsCmd));
	}
	...
	function WebSocketAdaptor() {
		...
		wsConn.onmessage = function(event) {
			...
			else if(obj.command == "askforFloor")
			{
				thiz.callback(obj.definition, obj);
			}
			...
			else if(obj.command == "getBackFloor")
			{
				thiz.callback(obj.definition,obj);			
			}
